...
class MultiHeadExternalAttention(nn.Module):
    ...
    def __init__(self, c, heads=4, dim_head=64, dropout=0.1):
        super(MultiHeadExternalAttention, self).__init__()
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(c, c, 1, bias=False),
            nn.BatchNorm2d(c),
            # nn.LayerNorm(c)  # Add LayerNorm for stability
        )
...
class BasicTransformerBlock_UNET(nn.Module):
    ATTENTION_MODES = {
    ...
    def _forward(self, x, context=None):
        ...
        # if isinstance(self.attn1, SelectiveAttention):
            ...
            # Assuming x is the input tensor to the layer_norm
            # normalized_shape = (x.shape[1], x.shape[2], x.shape[3])  # Dynamically set the normalized shape
            # x = F.layer_norm(x, normalized_shape)
            x = self.attn1(self.norm1(x)) + x

            # x = self.attn1(self.norm1(x)) + x  //origin
            # x = x.permute(2,1,0)
...
class SpatialTransformer_UNET(nn.Module):
    def __init__(self, in_channels, n_heads, d_head,
                 depth=1, dropout=0., context_dim=None,
                 disable_self_attn=False, use_linear=False,
                 use_checkpoint=True, use_external_attention=True):
        ...
        self.transformer_blocks = nn.ModuleList(
            [BasicTransformerBlock_UNET(inner_dim, n_heads, d_head, dropout=dropout,
                                   context_dim=context_dim[d] if context_dim else None,
                                   disable_self_attn=disable_self_attn, checkpoint=use_checkpoint)
             for d in range(depth)]
        )

...
from .openaimodel_hybrid import ResBlock, TimestepBlock, TimestepEmbedSequential, UNetModel, UNetModel_sige
    ...
    def __init__(
        ...
        if isinstance(num_res_blocks, int):
            ...
            if len(num_res_blocks) != len(channel_mult):
                raise ValueError("Provide num_res_blocks either as an int or as a list/tuple with the same length as channel_mult")
        ...
        # Add gate mechanism to input blocks and output blocks
        ...
        # Add gate mechanism to ResBlocks
        for level, mult in enumerate(channel_mult):
            for i in range(self.num_res_blocks[level]):
                layers = [
                    GateModule(ch),  # Adding GateModule here before ResBlock
                    SIGEResBlock(
                        ch,
                        time_embed_dim,
                        dropout,
        ...
        self.middle_block = TimestepEmbedSequential(
            SIGEResBlock(ch, time_embed_dim, dropout, dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm),
            SIGESpatialTransformer(
                ch,
                num_heads,
                dim_head,
        ...
        # Output blocks with gate mechanism added
        self.output_blocks = nn.ModuleList([])
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(self.num_res_blocks[level] + 1):
                ich = input_block_chans.pop()
                layers = [
                    GateModule(ch + ich),  # Adding GateModule here before ResBlock in the output blocks
                    SIGEResBlock(
                        ch + ich,
                        time_embed_dim,
                        dropout,
...
class SIGEUNetModel_origin(SIGEModel, UNetModel):
    def __init__(
        self,
        image_size,
        in_channels,

...
#         self.pruning_config = pruning_config if pruning_config is not None else {}
#         prune_input_indices = self.pruning_config.get('input_blocks', [])
#         prune_output_indices = self.pruning_config.get('output_blocks', [])
...
#                     # Check if the current block index should be pruned
#                     if current_block_idx not in prune_input_indices:
#                         self.input_blocks.append(TimestepEmbedSequential(*layers))
#                         self._feature_size += ch
#                         input_block_chans.append(ch)
#                     else:
#                         print(f"Pruning UNetModel input_block at index {current_block_idx}")

#                     current_block_idx += 1 # Increment index for each logical block

#             if level != len(channel_mult) - 1:
#                 out_ch = ch

#                 downsample_layer = TimestepEmbedSequential(
#                     ResBlock(
#                         ch,
#                         time_embed_dim,
#                         dropout,
...
#                 # 2. Then, wrap the append() and state updates in the pruning condition.
#                 if current_block_idx not in prune_input_indices:
#                     self.input_blocks.append(downsample_layer)
#                     ch = out_ch
#                     input_block_chans.append(ch)
#                     ds *= 2
#                     self._feature_size += ch
...
#                     print(f"INFO: Pruning UNetModel input_block (Downsample) at index {current_block_idx}")

#                 # 3. Increment the counter outside the if/else to ensure it always advances.
#                 current_block_idx += 1
...
#         current_block_idx = 0 # Reset for output blocks


#         for level, mult in list(enumerate(channel_mult))[::-1]:  # 0, 1, 2, 3,  1, 2, 4, 4
#             for i in range(self.num_res_blocks[level] + 1):
#                 if current_block_idx not in prune_output_indices:
#                     ich = input_block_chans.pop() # Pop only when we are actually building the block
#                     layers = [
#                         ResBlock(
#                             ch + ich,
#                             time_embed_dim,
...
#                     self._feature_size += ch
#                 else:
#                     # If we prune an output block, we must also discard its corresponding input channel info
#                     input_block_chans.pop() # Pop and discard to maintain sync
#                     print(f"INFO: Pruning UNetModel output_block at index {current_block_idx}")
...
# In your openaimodel.py, replace the entire UNetModel class with this FINAL version.

class UNetModel(nn.Module):
    def __init__(
        self,
        image_size, in_channels, model_channels, out_channels, num_res_blocks,
        attention_resolutions, dropout=0, channel_mult=(1, 2, 4, 4),
        conv_resample=True, dims=2, num_classes=None, use_checkpoint=False,
        use_fp16=False, use_bf16=False, num_heads=-1, num_head_channels=-1,
        ...
        adm_in_channels=None, pruning_config=None
    ):
        super().__init__()
        ...
        if context_dim is not None and type(context_dim) == ListConfig: context_dim = list(context_dim)
        ...
        for level, mult in enumerate(channel_mult):
            ...
            for nr in range(self.num_res_blocks[level]):
                ...
                # ch = mult * model_channels <- REMOVE THIS LINE
                if ds in attention_resolutions:
                    new_ch = mult * model_channels
                    if num_head_channels == -1: dim_head = new_ch // num_heads
                    else: num_heads = new_ch // num_head_channels; dim_head = num_head_channels
                    if legacy: dim_head = new_ch // num_heads if use_spatial_transformer else num_head_channels
                    if exists(disable_self_attentions): disabled_sa = disable_self_attentions[level]
                    else: disabled_sa = False
                    if not exists(num_attention_blocks) or nr < num_attention_blocks[level]:
                        layers.append(
                            SpatialTransformer(
                                new_ch, num_heads, dim_head, depth=transformer_depth, context_dim=context_dim,
                                disable_self_attn=disabled_sa, use_linear=use_linear_in_transformer,
                                use_checkpoint=use_checkpoint
                            )
                        )
                ...
                if current_block_idx not in prune_input_indices:
                    self.input_blocks.append(TimestepEmbedSequential(*layers))
                    ch = mult * model_channels # FIX: Update ch ONLY if the block is NOT pruned.
                    self._feature_size += ch
                    input_block_chans.append(ch)
                else:
                    print(f"INFO: Pruning UNetModel input_block at index {current_block_idx}")
                current_block_idx += 1
            # for nr in range(self.num_res_blocks[level]):
            #     layers = [ResBlock(ch, time_embed_dim, dropout, out_channels=mult * model_channels, dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm)]
            #     ch = mult * model_channels
            #     if ds in attention_resolutions:
            #         # This is the detailed logic you were right about. It is now fully included.
            #         if num_head_channels == -1: dim_head = ch // num_heads
            #         else: num_heads = ch // num_head_channels; dim_head = num_head_channels
            ...
            #         self.input_blocks.append(TimestepEmbedSequential(*layers))
            #         self._feature_size += ch
            #         input_block_chans.append(ch) # Only append if not pruned
            #     else:
            #         print(f"INFO: Pruning UNetModel input_block at index {current_block_idx}")
            #     current_block_idx += 1
            ...
            # if level != len(channel_mult) - 1:
            #     out_ch = ch
            #     downsample_layer = TimestepEmbedSequential(ResBlock(ch, time_embed_dim, dropout, out_channels=out_ch, dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm, down=True) if resblock_updown else Downsample(ch, conv_resample, dims=dims, out_channels=out_ch))
            #     if current_block_idx not in prune_input_indices:
            #         self.input_blocks.append(downsample_layer)
            #         ch = out_ch
            #         input_block_chans.append(ch) # Only append if not pruned
            ...
            if level != len(channel_mult) - 1:
                ...
                if current_block_idx not in prune_input_indices:
                    self.input_blocks.append(downsample_layer)
                    input_block_chans.append(out_ch) # Append if not pruned
                    self._feature_size += out_ch
                else:
                    print(f"INFO: Pruning UNetModel input_block (Downsample) at index {current_block_idx}")
                
                # --- CORRECTED LOGIC ---
                # Always update the channel count and downsample factor
                ch = out_ch
                ds *= 2
                current_block_idx += 1
        ...
        self.middle_block = TimestepEmbedSequential(
            ...
            SpatialTransformer(ch, num_heads, dim_head, depth=transformer_depth, context_dim=context_dim, use_linear=use_linear_in_transformer, use_checkpoint=use_checkpoint),
            ResBlock(ch, time_embed_dim, dropout, dims=dims, use_checkpoint=use_checkpoint),
        )
        self._feature_size += ch
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(self.num_res_blocks[level] + 1):
                ...
                ich = input_block_chans.pop()
                if current_block_idx not in prune_output_indices:
                    # If the block is NOT pruned, we use the 'ich' we just popped.
                    ...
                    # If the block IS pruned, the popped 'ich' is simply discarded.
                    # The important thing is that the list is now correct for the next iteration.
                    print(f"INFO: Pruning UNetModel output_block at index {current_block_idx}")
    ...
    def forward(self, x, timesteps=None, context=None, y=None, return_features=False, **kwargs):
        # This forward method is correct.
        hs = []
        features_for_distillation = []
        ...
        for module in self.input_blocks:
            ...
            hs.append(h)
            features_for_distillation.append(h)


        h = self.middle_block(h, emb, context)
        features_for_distillation.append(h)

        # --- NEW ROBUST FORWARD LOGIC ---
        hs_reverse = list(reversed(hs)) # Reverse the list to treat it like a queue
        output_block_hs_idx = 0
        for i, module in enumerate(self.output_blocks):
            # Check how many input channels the ResBlock in this module expects
            # This is a robust way to know if this block uses a skip connection
            resblock_in_channels = module[0].channels
            # Get the channel count of the tensor from the upsampling path
            h_channels = h.shape[1]

            if resblock_in_channels > h_channels:
                # If the ResBlock expects more channels than h has, it needs a skip connection
                skip_h = hs_reverse[output_block_hs_idx]
                h = th.cat([h, skip_h], dim=1)
                output_block_hs_idx += 1
        ...
        output = self.out(h)
        if return_features:
            return output, features_for_distillation
        return output

...
class MultiHeadExternalAttention(nn.Module):
    def __init__(self, in_channels, coef=1, M=64, num_heads=4, use_cross_kv=False):
        ...
        self.k_proj = nn.Conv2d(in_channels, self.inter_channels, kernel_size=1, stride=1, padding=0)
        self.v_proj = nn.Conv2d(self.inter_channels, in_channels, kernel_size=1, stride=1, padding=0)

        if self.use_cross_kv:
            self.cross_k_proj = nn.Conv2d(in_channels, self.inter_channels, kernel_size=1, stride=1, padding=0)
            self.cross_v_proj = nn.Conv2d(self.inter_channels, in_channels, kernel_size=1, stride=1, padding=0)

        self.norm = nn.LayerNorm(in_channels)  # Use LayerNorm for stability
    ...
    def _normalize_attention(self, attn, dim=-1, epsilon=1e-8):
        """
        Normalize attention weights along the specified dimension.
        """
        attn_sum = attn.sum(dim=dim, keepdim=True)
        return attn / (attn_sum + epsilon)  # Avoid division by zero
    ...
    def forward(self, x, cross_k=None, cross_v=None):
        ...
        # Normalize input
        x = self.norm(x)  # Normalize input (B, N, C)
        x = x.permute(0, 2, 1).contiguous().view(B, C, 1, N)  # Reshape to (B, C, 1, N)

        if not self.use_cross_kv:
            # Key projection
            k_out = self.k_proj(x)  # (B, Inter_C, 1, N)
            k_out = self._normalize_attention(k_out.squeeze(2), dim=-1)  # Normalize along spatial dim (N)
            k_out = self._normalize_attention(k_out, dim=-2)  # Normalize along channel dim (Inter_C)
            ...
            cross_k_out = self.cross_k_proj(x)  # Cross-key projection
            cross_k_out = self._normalize_attention(cross_k_out.squeeze(2), dim=-1)
            cross_k_out = self._normalize_attention(cross_k_out, dim=-2)

            # Cross-value projection
            v_out = self.cross_v_proj(cross_k_out.unsqueeze(2)).squeeze(2)  # Cross-value projection
...
#         self.k_proj = nn.Linear(in_channels, self.inter_channels)
#         self.v_proj = nn.Linear(self.inter_channels, in_channels)

#         if self.use_cross_kv:
#             self.cross_k_proj = nn.Linear(in_channels, self.inter_channels)
#             self.cross_v_proj = nn.Linear(self.inter_channels, in_channels)
...
#     def stable_softmax(self, logits, dim=-1):
#         """
...
#         # Logsumexp trick for stability
#         log_exp_x = x - torch.max(x, dim=3, keepdim=True)[0]  # Subtract max for numerical stability
#         log_exp_x = torch.exp(log_exp_x)  # Exponentiate the adjusted logits
#         log_sum_exp = torch.log(torch.sum(log_exp_x, dim=3, keepdim=True) + epsilon)  # Log-sum-exp
#         x = log_exp_x - log_sum_exp  # Log-probabilities after logsumexp trick
...
#     def forward(self, x, cross_k=None, cross_v=None):
#         x = self.norm(x.permute(0, 2, 1)).permute(0, 2, 1)  # Normalize input (B, N, C)

#         if not self.use_cross_kv:
#             # Project key and value
#             k_out = self.k_proj(x)  # (B, N, Inter_C)
#             k_out = self._act_dn(k_out.transpose(1, 2))  # Apply attention activation
#             v_out = self.v_proj(k_out.transpose(1, 2))  # (B, N, C)
#         else:
#             assert cross_k is not None and cross_v is not None, \
#                 "cross_k and cross_v must be provided when use_cross_kv=True"
#             cross_k_out = self.cross_k_proj(x)  # (B, N, Inter_C)
#             cross_k_out = self._act_dn(cross_k_out.transpose(1, 2))
#             v_out = self.cross_v_proj(cross_k_out.transpose(1, 2))  # (B, N, C)
...
#         return v_out  # (B, N, C)
    ...
    #     x = x.view(B, N, self.num_heads, -1)  # B, N, num_heads, coef // num_heads

    #     # Prepare input for Conv2d
    #     x = x.permute(0, 2, 1, 3)  # B, num_heads, N, coef // num_heads
    #     x = x.reshape(B * self.num_heads, 1, N, -1)  # Merge batch and num_heads

    #     # Dynamically set Conv2d kernel size
    #     if self.conv_0 is None or x.shape[-1] != self.k:
    #         self.k = min(self.k, x.shape[-1])  # Ensure kernel size <= input width
    #         self.conv_0 = nn.Conv2d(1, self.num_heads, kernel_size=(1, self.k), stride=1, padding=0).to(x.device)

    #     # Apply Conv2d
    #     attn = self.conv_0(x)  # (B * num_heads, num_heads, N, k)

    #     # Softmax and attention dropout
    #     attn = self.stable_softmax(attn, dim=-2)
    #     attn = attn / (1e-9 + attn.sum(dim=-1, keepdim=True))
    #     attn = self.attn_drop(attn)

    #     # Reshape and apply linear_1
    #     attn = attn.view(B, self.num_heads, N, -1).permute(0, 2, 1, 3).reshape(B, N, -1)
    #     x = self.linear_1(attn)  # B, N, dim * coef // num_heads
...
class BasicTransformerBlock(nn.Module):
    ...
    def _forward(self, x, context=None):
        ...
        if isinstance(self.attn2, MultiHeadExternalAttention):
            ...
            # x = self.ff_EA(self.norm3(x).permute(2,1,0)) + x  # 残差连接
        else:
            attn2_output = self.attn2(self.norm2(x))
            x = attn2_output + x  # 残差连接
